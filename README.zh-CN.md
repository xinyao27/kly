...
- **依赖图可视化** — 将文件依赖关系渲染为 Mermaid 图（ASCII、SVG 或原始语法）
- **LLM Rerank** — 可选使用 LLM 对搜索结果进行语义重排序，提升相关性
- **简洁 CLI** — `init`、`build`、`query`、`show`、`overview`、`graph`、`serve`、`hook`、`gc`
...
- `search_files` — 自然语言文件搜索（FTS5 全文搜索，可选 LLM 重排序）
