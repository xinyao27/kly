...
- **100% test coverage** — All automatable code must achieve 100% coverage (statements, branches, functions, lines). Coverage enforced via `npm run test:coverage`
- **TDD (Test-Driven Development)** — New features must have tests written first, then implementation. See [Testing Documentation](testing.md) for details
...
- `typescript.ts` — Handles TS/TSX/JS/JSX via `tree-sitter-typescript` (JS is a subset of TS, so a single parser handles both)
