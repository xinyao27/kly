...
Implemented complete MCP (Model Context Protocol) adapter for clai, enabling clai apps to be exposed as MCP servers for Claude Desktop/Code integration. Single-app architecture with automatic schema conversion and zero-config setup.
...
3. **CLI Integration** (`bin/clai.ts`)
   - Added `clai mcp <target>` subcommand
   - Works with local files and remote GitHub repos
   - Sets `CLAI_MCP_MODE=true` environment variable
...
5. **Remote Support** (`src/remote/`)
   ...
   - Enables: `clai mcp user/repo[@ref]`
...
# Remote repos
clai mcp user/repo
clai mcp user/repo@v1.0.0
clai mcp user/repo@branch

# With cache refresh
clai mcp user/repo --force
```
...
Each clai app runs as its own MCP server. Simple, isolated, easy to configure.
...
### Modified Files
- `bin/clai.ts` - Added `mcp` subcommand, `runFileAsMcp()`
