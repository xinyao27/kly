...
/**
 * Get the path to clai.sum file
 */
export function getSumFilePath(): string {
  return join(homedir(), ".clai", "clai.sum");
}

/**
 * Manager for clai.sum file (integrity verification database)
 *
...
export class SumFileManager {
  ...
  /**
   * Load entries from clai.sum file
   */
  private load(): void {
    ...
    try {
      ...
      console.warn(`Warning: Failed to load clai.sum: ${error}`);
    }
  }

  /**
   * Parse a single line from clai.sum
   */
  ...
  private save(): void {
    ...
    try {
      ...
      console.error(`Error: Failed to save clai.sum: ${error}`);
      throw error;
    }
  }
  ...
  /**
   * Add or update an entry in clai.sum
   *
   * @param url - Full URL
